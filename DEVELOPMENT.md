...
Voice provider note: server-side V voice lives under `GET /v1/voice/status`, `POST /v1/voice/speech`, and `POST /v1/voice/transcribe`. `createVoiceClient()` in `packages/agents` owns provider switching, TTS caching, ElevenLabs/OpenAI TTS, OpenAI/Azure/Gemini STT, and server-only credentials. `AI_PROVIDER` / `AI_MODEL*` generate V's text only; `VOICE_STT_PROVIDER` hears the user, and `VOICE_TTS_PROVIDER` speaks V's reply. Local dev can leave voice providers disabled; the web apps check voice status and fall back to browser speech where possible. TTS cache keys include provider, model, voice, ElevenLabs voice settings, style version, format, and text; bump `VOICE_TTS_STYLE_VERSION` whenever V's locked voice style changes so cached audio regenerates.
...
Current voice input/output: the public site, employer web, worker web and admin console use shared browser capture from `@viora/ui`, call `/v1/voice/transcribe` for STT, and fall back to browser `SpeechRecognition` only when server capture/transcription is unavailable. The public site and admin console call `/v1/voice/speech` first for V replies; browser `speechSynthesis` remains a fallback only when server TTS is disabled or unavailable.

Voice env:
- `VOICE_TTS_PROVIDER` - `disabled` locally, `elevenlabs` for production V voice, or `openai` if using OpenAI TTS
- `VOICE_TTS_STYLE` - spoken style contract for OpenAI TTS and fallback voice behavior; ElevenLabs delivery is mainly controlled by the saved voice, text, and voice settings
